...
def main():
    ...
    for line in args.infile:
        ...
        assert len(fields) == num_cols, f"Wrong number of columns. Found '{len(fields)}', expected '{num_cols}'"
        if num_cols == 2:
            ...
            assert link.samefile(src), f"Link '{link}' does not point to '{src}'"
...
def parseOpts():
    parser = argparse.ArgumentParser(description='Move files but leave a symbolic link at the old location')
    parser.add_argument('-t', '--test', action="store_true", help='Make test files and exit')
    parser.add_argument('-v', '--verbose', action="store_true", help='Print actions taken')
    parser.add_argument('-2', '--two-column', action="store_true", help='Use 2-column mode')
    parser.add_argument('-3', '--three-column', action="store_true", help='Use 3-column mode')
    parser.add_argument('-n', '--dry-run', action="store_true",
                        help="Print what would be done, but don't do it")
    parser.add_argument('infile', nargs='?', type=argparse.FileType('r'),
                        default=sys.stdin, help="Read from files or stdin")
