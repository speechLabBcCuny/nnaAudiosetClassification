...
while True:
    ...
    files_to_do = set(files_in_pre_queu).difference(
        set(files_in_processing)).difference(set(files_done))
    files_to_do = list(files_to_do)[:file_batch_size]
    if files_to_do:
        # save to processing queue
        pre_process_func.save_to_csv(VGGISH_processing_queue,
                                     [[str(afile)] for afile in files_to_do])
        for npy_file in files_to_do:
            embeddings_file_path = vgg.inference_file(npy_file)
            pre_process_func.save_to_csv(VGGISH_EMBEDDINGS_queue,
                                         [[str(npy_file)]])
