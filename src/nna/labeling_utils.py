...
class labeling_UI:
    ...
    def init_UI(self):
        ...
        for checkbox, button in self.items.items():
            if 'TagButton' in checkbox:
                button.on_click(self.reverse_square_box)

    def update_row_with_UI_info(self):
        labels_from_UI = {}
        for checkbox, button in self.items.items():
            if 'TagButton' in checkbox:
                if button.icon == 'check-square':
                    labels_from_UI[button.description] = '1'
                elif button.icon == 'square':
                    labels_from_UI[button.description] = '0'
                else:
