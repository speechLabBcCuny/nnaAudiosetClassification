...
def data_to_samples(sound_ins,excerpt_len):
    ...
    if data_len_sec<excerpt_len:
        missing_element_count = int(min_expected_element_count - sound_ins.data.size)
        padded_data = np.pad(sound_ins.data , (0,missing_element_count), 'constant', constant_values=(0, 0))
        ...
        samples = sound_ins.data[:data_trim_point].reshape(-1,int(excerpt_sample_size))
        sample_list = []
        ...
        if left_over_len >=5:
            missing_element_count = int(min_expected_element_count - (sound_ins.data.size-data_trim_point))
            padded_data = np.pad(sound_ins.data[data_trim_point:] , (0,missing_element_count), 'constant', constant_values=(0, 0))
...
def prepare_run_inputs(config, X_train, X_test, X_val, y_train, y_test, y_val):
    ...
    to_tensor = modelarchs.ToTensor(runconfigs.MAX_MEL_LEN,runconfigs.SAMPLING_RATE,config['device'])
...
#silence,rain,insect,grouse-ptarmigan
