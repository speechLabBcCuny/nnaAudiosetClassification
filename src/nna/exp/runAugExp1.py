...
##### RAW DATA
    ...
    def augmentData(data,sr,config):
        ...
        if config["aug_ID"]==0:
            dataAug = augmentations.pitch_shift(data,sr, config["pitch_shift_n_steps"])
        elif  config["aug_ID"]==1:
            output_length=44100*10
            dataAug = augmentations.time_stretch(data,output_length, config["time_stretch_factor"])
    ...
    X_train=np.concatenate([X_train,]+res)
    ...
    ##### dataloaders
    sound_datasets = {phase: runUtils.audioDataset(XY[0],XY[1])
