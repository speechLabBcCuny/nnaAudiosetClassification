...
for i in range(0, len(temp), file_per_epoch):
  ...
  for input_file in epoch_files:
    segments_folder, embeddings_file_name, pre_processed_folder = preb_names(
        input_file, OUTPUT_DIR, INPUT_DIR_PARENT)
    if os.path.exists(embeddings_file_name):
      continue
    pre_processed_npy_files = [pre_processed_folder +
                               file for file in os.listdir(pre_processed_folder)]
    # print(pre_processed_npy_files)
    embeddings_file_name = inference(
