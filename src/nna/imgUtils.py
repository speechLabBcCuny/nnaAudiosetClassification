...
def readTimeLapseDatabase(pathDatasets, ):
  ...
  find all files ending with .ddb at pathDatasets,
  read dataTable and store in pandas dataFrame
  ...
  """
    datasetList = [[m for m in i.glob("*") if ".ddb" in str(m)][0]
                   for i in (Path(pathDatasets).glob("*"))]
    ...
    for location, path2dataset in zip(labeledImgLocations, datasetList):
        ...
        start_times = start_times.apply(
            lambda x: datetime.datetime.strptime(x, "%d-%b-%Y_%H:%M:%S"))
...
def imgDataTable2Numpy(dictDataTable):
    ...
    for dataTableID in dictDataTable_new_index:
        dict_numpy_tables[dataTableID] = {}

        previousLen = len(dictDataTable_new_index[dataTableID])
        dictDataTable_new_index[dataTableID] = dictDataTable_new_index[
            dataTableID].set_index(["timestamp"], )
        dictDataTable_new_index[dataTableID] = dictDataTable_new_index[
            dataTableID].sort_index()

        caribou_np = dictDataTable_new_index[dataTableID]["Caribou"].values
        timeIndex_np = pd.Series(
            dictDataTable_new_index[dataTableID].index).values

        dict_numpy_tables[dataTableID]["Caribou"] = caribou_np
        dict_numpy_tables[dataTableID]["timestamp"] = timeIndex_np
