...
def preb_names(mp3_file_path,
               output_dicretory=OUTPUT_DIR,
               abs_input_path=INPUT_DIR_PARENT):
    mp3_file_path = Path(mp3_file_path)
    absolute_output_dir = relative2outputdir(mp3_file_path, output_dicretory,
                                             abs_input_path)
    segments_folder = absolute_output_dir.joinpath(mp3_file_path.stem +
                                                   "_segments/")
    embeddings_file_name = absolute_output_dir.joinpath(mp3_file_path.stem +
                                                        "_embeddings.npy")
    pre_processed_folder = absolute_output_dir.joinpath(mp3_file_path.stem +
                                                        "_preprocessed/")
...
def cal_sample_size(wav_data, sr):
  ...
  """
    ...
    if remainder_wav_data < lower_limit:
        ...
        seconds = math.floor(
            ((remainder_wav_data * sampling_ratio) - (240)) / (16000 * 0.96))
        sample_size += seconds
...
def iterate_for_waveform_to_examples(wav_data, sr):
  ...
  """
    ...
    for i in range(0, len(wav_data), offset):
        ...
        a_sound = vggish_input.waveform_to_examples(wav_data[i:i + (offset)],
                                                    sr)
...
def pre_process(mp3_file_path, output_dir="./", saveAsFile=False):
  ...
  """
    ...
    npy_file_path = Path(output_dir) / \
        (str(mp3_file_path.stem) + "_preprocessed.npy")
...
def pre_process_big_file(mp3_file_path,
  ...
  """
    ...
    pre_processed_dir = Path(output_dir) / \
        (mp3_file_path.stem + "_preprocessed/")
    ...
    for mp3_segment in mp3_segments:
        ...
        if (str(npy_file_path)
                not in files_in_vggprocessing) and (str(npy_file_path)
