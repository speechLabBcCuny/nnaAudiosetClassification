...
def divide_mp3(mp3_file_path,segments_folder,segment_len="01:00:00"):
    ...
    sp.wait()
    print("processssssseing")
    mp3_segments=os.listdir(segments_folder)
    return mp3_segments


def pre_process(mp3_segment,segments_folder,i, output):
...
def end2end(input_file,output_folder,abs_input_path,sess,vgg,pproc,cpu_count=2):
...
#   print("pre_process")
  ...
  sounds=[]
  print("len mp3segments",len(mp3_segments))
  for k in range(0,len(mp3_segments),cpu_count):
    print("batch",k)
    output = mp.Queue()
    sys.stdout.flush()
    processes = [mp.Process(target=pre_process, args=(mp3_segment,segments_folder,i, output))  for (i,mp3_segment) in enumerate(mp3_segments[k:k+cpu_count])]
    for p in processes:
        p.start()
    for p in processes:
        p.join()
    results = [output.get() for p in processes]
    print("first_results",len(results))
    results.sort()
...
if __name__ == "__main__":
    ...
    for i,input_file in enumerate(input_files):
        ...
        end2end(input_file,output_folder,abs_input_path,sess,vgg,pproc,cpu_count=8)
        end=time()
