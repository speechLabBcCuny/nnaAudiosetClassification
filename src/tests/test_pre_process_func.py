...
def test_relative2outputdir():
    ...
    expected_absolute_output_dir=Path('/output/directory/18 Fish Creek 4/July 2016/')

    absolute_output_dir=pre_process_func.relative2outputdir(mp3_file_path,output_dir)
...
def test_preb_names():
    ...
    output=pre_process_func.preb_names(mp3_file_path,
                                                output_dicretory,abs_input_path)
    ...
    expected_output=(segments_folder,embeddings_file_name,pre_processed_folder)
...
def test_divide_mp3():
    ...
    mp3_segments=pre_process_func.divide_mp3(mp3_file_path,segments_folder,
                                                segment_len="01:00:00")
    assert(len(mp3_segments)==4)

    args=("ffprobe","-show_entries", "format=duration","-i","tests/data/segments/output000.mp3")
    ...
    args=("ffprobe","-show_entries", "format=duration","-i","tests/data/segments/output003.mp3")
    popen = Popen(args, stdout = PIPE)
    popen.wait()
    output = popen.stdout.read()
    length_min=str(output).split("\\n")[1].split("=")[1].split(".")[0]
