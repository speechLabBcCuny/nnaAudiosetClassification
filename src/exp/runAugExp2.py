...
#   DEFAULT values
    ...
    tagSet=["Songbird","Water Bird","Insect",
            "Running Water","Rain","Cable","Wind","Aircraft"]
...
##### RAW DATA
    ...
    pitch = augmentations.pitch_shift_n_stepsClass(44100, config["pitch_shift_n_steps"])
    noise = augmentations.addNoiseClass(config["noise_factor"])
    strech = augmentations.time_stretchClass(441000, config["time_stretch_factor"],isRandom=True)
    shift = augmentations.shiftClass(config["roll_rate"],isRandom=True)
    toTensor = augmentations.ToTensor(850)

    transformCompose = transforms.Compose([pitch,
                                        strech,
                                        shift,
                                        noise,
                                        toTensor,])
    ...
    sound_datasets = {phase: runUtils.audioDataset(XY[0],XY[1],transform=transformCompose)
    ...
    if config.CNNLayer_count==1:
        model = modelArchs.NetCNN1(config.CNN_filters_1,h_w,kernel_size).float().to(device)

    if config.CNNLayer_count==2:
        model = modelArchs.NetCNN2(config.CNN_filters_1,config.CNN_filters_2,h_w,kernel_size,kernel_size).float().to(device)
    ...
    print("ready ?")
    runUtils.run(model, dataloaders,optimizer,criterion,metrics,device,config,wandb_project_name)
