...
def time_stretch(data,output_length, time_stretch_factor,singleElement=False):
    ...
    def strecth(y,output_length, time_stretch_factor):
        dataAug = librosa.effects.time_stretch(y.reshape(-1), time_stretch_factor)
        dataAug = dataAug.astype(data.dtype)

        if len(dataAug)>output_length:
            dataAug = dataAug[:output_length]
        else:
            dataAug = np.pad(dataAug, (0, max(0, output_length - len(dataAug))), "constant")
    ...
    for index,y in enumerate(data):
        strecth(data,output_length, time_stretch_factor)
        if index==0:
...
class time_stretchClass(object):
    ...
    def __call__(self, sample):
        ...
        if self.isRandom:
            ...
            time_stretch_factor_val = (upper - lower) * np.random.random_sample() + lower
        else:
            time_stretch_factor_val = self.time_stretch_factor
        x = time_stretch(x,self.output_length, time_stretch_factor_val,
                                singleElement=True)
        return x,y
