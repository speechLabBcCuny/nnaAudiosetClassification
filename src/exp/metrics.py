...
def roc_auc_perClass_compute_fn(y_preds, y_targets):
    ...
    for y_true_perClass_Index in y_true.shape[1]:
        res.append(roc_auc_score(y_true[:,y_true_perClass_Index], y_pred[:,y_true_perClass_Index]))

    return res

[docs]class ROC_AUC_perClass(EpochMetric):
    ...
    def __init__(self, output_transform=lambda x: x, check_compute_fn: bool = False):
        super(ROC_AUC_perClass, self).__init__(
            roc_auc_perClass_compute_fn, output_transform=output_transform, check_compute_fn=check_compute_fn
        )
